...
def ToNum(numst):
    ...
    elif lenfrac[1] in numst or lenfrac[2] in numst or lenfrac[3] in numst or lenfrac[4] in numst or lenfrac[
        5] in numst or lenfrac[6] in numst:
        fractp = numst
...
def CheckFracPeriod(numfr):
    for i in range(4, len(numfr)):
        if numfr[i] == numfr[i - 1] == numfr[i - 2] == numfr[i - 3] == numfr[i - 4] and numfr[i] != '0':
            return [numfr[i], i - 4]
        elif [numfr[i], numfr[i - 1]] == [numfr[i - 2], numfr[i - 3]] == [numfr[i - 4], numfr[i - 5]] != ['0',
                                                                                                          '0'] and i >= 5:
            return [f'{numfr[i - 1]}{numfr[i]}', i - 5]
        elif [numfr[i], numfr[i - 1], numfr[i - 2]] == [numfr[i - 3], numfr[i - 4], numfr[i - 5]] == [numfr[i - 6],
                                                                                                      numfr[i - 7],
                                                                                                      numfr[i - 8]] != [
            '0', '0', '0'] and i >= 8:
            return [f'{numfr[i - 2]}{numfr[i - 1]}{numfr[i]}', i - 8]
        elif [numfr[i], numfr[i - 1], numfr[i - 2], numfr[i - 3]] == [numfr[i - 4], numfr[i - 5], numfr[i - 6],
                                                                      numfr[i - 7]] == [numfr[i - 8], numfr[i - 9],
                                                                                        numfr[i - 10],
                                                                                        numfr[i - 11]] != ['0', '0',
                                                                                                           '0',
                                                                                                           '0'] and i >= 11:
...
def PrintNumb(s):
    ...
    if fr == '':
        ...
        if lenfr != 0 and per[0] != '0':
            return (' '.join(p[::-1]) + ' и ' + PrintNumb(beforper) + ' ' + lenfrac[lenfr] + ' и ' + PrintNumb(
                per) + ' в периоде').replace('  ', ' ')
        elif lenfr != 0 and per[0] == '0':
            return (' '.join(p[::-1]) + ' и ' + PrintNumb(beforper) + ' ' + lenfrac[lenfr] + ' и ноль ' + PrintNumb(
                per) + ' в периоде').replace('  ', ' ')
        elif per[0] != '0':
...
print(WordCalculator(
    'девятнадцать и восемьдесят две сотых разделить на скобка открывается девяносто девять минус один скобка закрывается'))
